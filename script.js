/**
 * Fish Simulator
 * High-Quality 8-Bit Pixel Fishing Simulation
 */
...
function resize() {
    // Integer-aware scaling for crisp pixels
    const sw = window.innerWidth;
    const sh = window.innerHeight;
    
    CANVAS_WIDTH = Math.floor(sw / PIXEL_SCALE);
    CANVAS_HEIGHT = Math.floor(sh / PIXEL_SCALE);
    
    canvas.width = CANVAS_WIDTH;
    canvas.height = CANVAS_HEIGHT;
    
    // Set exact CSS size to prevent blurring
    ...
    for (let i = 0; i <= CANVAS_WIDTH; i++) {
        GRAVEL_MAP[i] = CANVAS_HEIGHT - 12 + Math.sin(i * 0.08) * 3;
    }
}
...
const FISH_TYPES = {
    ...
    shrimp: { c1: '#ffffff', c2: '#ffbbbb', c3: '#000000', outline: '#000000', size: 12, speed: 0.4, habitat: 'bottom' },
...
class Plant {
    ...
    draw() {
        ...
        if (this.type === 'anemone') {
            ...
            ctx.fillStyle = this.color;
            ctx.fillRect(this.x - 4, this.y - 2, 8, 3);
            ctx.fillStyle = '#88dd88';
            ctx.fillRect(this.x - 2, this.y - 3, 4, 1);
            ctx.strokeStyle = 'rgba(255,255,255,0.3)';
            ctx.beginPath(); ctx.moveTo(this.x, this.y + 1); ctx.lineTo(this.x, this.y + 6); ctx.stroke();
        } else {
            ctx.fillStyle = this.color;
            const segments = 4;
...
class Fish {
    ...
    update() {
        ...
        if (this.x < -20 || this.x > CANVAS_WIDTH + 20 || this.y < -20 || this.y > CANVAS_HEIGHT + 20) {
            this.x = CANVAS_WIDTH / 2; this.y = CANVAS_HEIGHT / 2;
            this.targetX = this.x; this.targetY = this.y;
        }
            ...
            if (Math.abs(this.x - this.targetX) < 15 && Math.abs(this.y - this.targetY) < 15) {
                if (Math.random() < 0.05) { this.idleTimer = 40 + Math.random() * 80; }
                    ...
                    if (this.config.habitat === 'bottom') {
                        this.targetY = (GRAVEL_MAP[Math.floor(this.targetX)] || (CANVAS_HEIGHT - 15)) - 4;
                    } else {
    ...
    draw() {
        ...
        if (this.type === 'yellowtang' || this.type === 'bluetang') {
            dot(-4, -4, 9, 9, config.outline);
            dot(-3, -3, 7, 7, config.c1);
            if (this.type === 'yellowtang') {
                dot(2, -2, 2, 4, config.c2);
                dot(-2, -5, 4, 1, config.c3);
                dot(-2, 4, 4, 1, config.c3);
                dot(-7, -2 + sway/p, 3, 5, config.c3);
            } else {
                dot(-2, -2, 6, 4, config.c3);
                dot(4, 0, 2, 2, config.c2);
                dot(-8, -2 + sway/p, 3, 5, config.c2);
            }
            dot(3, -2, 1, 1, '#000');
        }
            ...
            dot(-2, -3, 2, 7, config.outline); dot(-1, -2, 1, 5, config.c2);
            dot(1, -3, 2, 7, config.outline); dot(2, -2, 1, 5, config.c2);
            dot(-7, -2 + sway/p, 3, 5, config.outline);
            dot(-6, -1 + sway/p, 2, 3, config.c1);
            dot(4, -1, 1, 1, '#000');
        }
        else if (this.type === 'angelfish' || this.type === 'discus') {
            dot(-2, -6, 5, 13, config.outline);
            dot(-4, -4, 9, 9, config.outline);
            dot(-1, -5, 3, 11, config.c1);
            dot(-3, -3, 7, 7, config.c1);
            if (this.type === 'angelfish') {
                dot(0, -5, 1, 11, config.c3); dot(2, -3, 1, 7, config.c3);
                dot(0, 6, 1, 4, config.c2);
            } else {
                dot(-1, -4, 2, 9, config.c2); dot(1, -3, 1, 7, config.c3);
            }
            dot(-6, -2 + sway/p, 3, 5, config.outline);
            dot(3, -2, 1, 1, '#000');
        }
        else if (this.type === 'goldfish' || this.type === 'pufferfish') {
            dot(-6, -4, 11, 9, config.outline);
            dot(-5, -3, 9, 7, config.c1);
            if (this.type === 'goldfish') {
                dot(-3, -5, 5, 2, config.outline);
                dot(-9, -6 + sway/p, 5, 13, config.outline);
                dot(-8, -5 + sway/p, 3, 11, config.c3);
            } else {
                dot(-5, 1, 9, 3, config.c2);
                dot(-3, -2, 2, 2, config.c3); dot(0, -1, 2, 2, config.c3);
                dot(-8, -1, 3, 3, config.outline);
            }
            dot(4, -2, 1, 1, '#000');
        }
        else if (this.type === 'koi' || this.type === 'oscar') {
            ...
            if (this.type === 'koi') {
                ...
                ctx.restore();
                dot(7, 1, 2, 2, config.outline);
                dot(-11, -3 + sway/p, 4, 7, config.outline);
            } else {
                dot(-2, -3, 4, 6, config.c2); dot(-8, -1, 2, 2, config.c2);
                dot(-10, -3 + sway/p, 4, 7, config.outline);
            }
            dot(6, -1, 1, 1, '#000');
        }
        else if (this.type === 'neon' || this.type === 'danio') {
            dot(-6, -2, 13, 5, config.outline);
            dot(-5, -1, 11, 3, config.c2);
            if (this.type === 'neon') {
                dot(-5, -1, 11, 1, config.c1); dot(-3, 0, 7, 2, config.c3);
            } else {
                dot(-5, -1, 11, 1, config.c2); dot(-5, 1, 11, 1, config.c2);
            }
            dot(-8, -2 + sway/p, 3, 5, (this.type === 'neon' ? config.c3 : config.outline));
            dot(5, -1, 1, 1, '#000');
        }
        else if (this.type === 'betta' || this.type === 'guppy' || this.type === 'molly' || this.type === 'platy') {
            dot(-4, -2, 8, 5, config.outline);
            dot(-3, -1, 6, 3, config.c1);
            if (this.type === 'betta') {
                ctx.globalAlpha = 0.8;
                dot(-4, -8 + sway/p, 7, 7, config.c2); dot(-4, 2 - sway/p, 7, 8, config.c2);
                dot(-12, -6 + sway/p*2, 9, 13, config.c2);
                ctx.globalAlpha = 1.0;
            } else if (this.type === 'guppy') {
                dot(-11, -5 + sway/p, 9, 11, config.outline); dot(-10, -4 + sway/p, 7, 9, config.c2);
            } else if (this.type === 'molly') {
                dot(-1, -6, 5, 4, config.outline); dot(0, -5, 3, 2, config.c2);
                dot(-8, -3 + sway/p, 4, 7, config.outline);
            } else {
                dot(-8, -3 + sway/p, 4, 7, config.c3); dot(-2, -6, 3, 3, config.c3);
            }
            dot(3, -1, 1, 1, '#000');
        }
        else if (this.type === 'cory' || this.type === 'shrimp' || this.type === 'coralshrimp' || this.type === 'snail') {
            if (this.type === 'snail') {
                dot(-4, -5, 8, 8, config.outline); dot(-3, -4, 6, 6, config.c1);
                dot(-2, -3, 4, 4, config.c3); dot(-6, 2, 12, 3, config.outline);
                dot(-5, 3, 10, 1, config.c2); dot(3, 0, 1, 2, config.outline);
                dot(5, 0, 1, 2, config.outline); dot(3, -1, 1, 1, '#000'); dot(5, -1, 1, 1, '#000');
            } else {
                dot(-5, -2, 10, 4, config.outline);
                if (this.type === 'cory') {
                    dot(-4, -1, 8, 3, config.c1); dot(4, 2, 2, 2, config.outline);
                    dot(-9, -2 + sway/p, 4, 5, config.outline);
                } else if (this.type === 'shrimp') {
                    dot(-4, -1, 8, 2, 'rgba(255,255,255,0.7)'); dot(4, -3, 5, 1, '#fff');
                    dot(-2, 2, 1, 2, config.outline); dot(1, 2, 1, 2, config.outline);
                } else {
                    dot(-4, -1, 2, 2, config.c2); dot(-2, -1, 2, 2, config.c1);
                    dot(0, -1, 2, 2, config.c2); dot(4, -3, 1, 4, '#fff');
                    dot(1, 2, 3, 1, config.c2); dot(-2, 2, 3, 1, config.c2);
                }
                dot(3, -1, 1, 1, '#000');
            }
        }
